...
\begin{algorithm}[H]
  \ForEach{Knoten im Graph}{
    \If{jedes Paar von Kanten in die gleiche Richtung
      uneindeutig geordnet ist mit Wahrscheinlichkeit 
      $>$ p-cutoff {\bf and} die Summe der estCopyNum $<$ 
      cn-cutoff ist}
       {markiere den Endknoten mit der kleineren 
	 estCopyNum als polymorph. Markiere Kante zum
	 polymorphen Knoten und alle Kanten vom polymorphen 
         Knoten, so dass sie im nächsten 
	 Schritt nicht mitbeachtet wird\;}
       Berechne den längsten Overlap von allen Paaren von
       Kanten in die gleiche Richtung, deren Endpunkt 
       nicht polymorph markiert ist.\;
       \If{längster Overlap $>$ 400} {
	 markiere alle Kanten und Rückkanten schwarz\;
       }
  }

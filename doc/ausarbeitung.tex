...
\begin{table}
  \adjustbox{max height=\dimexpr\textheight-5.5cm\relax, max width=\textwidth}{
    \begin{tabular}{llccccc}
      \toprule
      Datensatz & Programm & Gesamtlänge & Abdeckung (\%) & N50 (kb)
      & \#Missassemblierungen & Unaligniert (Mbp)
      (MB) \\
      \midrule
      \textit{S. cerevisiae}
      ...
      \bottomrule
  \end{tabular}}
  \caption{\label{tab: Vergleich}Ergebnisse der Sequenz-Rekonstruktion.
  Datensatz bezeichnet die Testdatensätze mit zugehöriger Anzahl der
  assemblierten Contigs, um die Größe des jeweiligen Scaffold-Graphen zu
  skizzieren. Für jeden Datensatz wurden die zunächst die durch SGA erzielten
  Ergebnisse aufgetragen in der ersten Zeile als absolute Werte aufgetragen,
  ...
  die prozentuale Abdeckung der Zielsequenz \todo{bessere Beschreibung für
  Abdeckung ausdenken} und N50 die Länge, sodass alle Scaffolds mit gleicher
  oder größerer Länge mindestens die Hälfte der Gesamtlänge ausmachen.
  \#Missassemblierungen bezeichnet die Anzahl der Missassemblierungen nach
  Plantagora \cite{Gurevich:2013je}, Unaligniert die Gesamtlänge nicht
  alignierter Regionen der Assemblierungen.}
\end{table}
...
\todo{absatz mit vergleich zur implementation in einer skript-sprache
  schreiben}

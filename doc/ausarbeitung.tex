...
beiden Strategien für 4 Testdatensätze (vgl.~\ref{sec: Testdaten}) ausgeführt.

Vergleich~\ref{tab: stddev}.

Zum Zeitpunkt dieser Ausarbeitung ist die MaxStdDev-Strategie in gt Scaffolder
implementiert.
...
\begin{table}
  \adjustbox{max height=\dimexpr\textheight-5.5cm\relax, max width=\textwidth}{
    \begin{tabular}{llccccc}
      ...
      \bottomrule
  \end{tabular}}
  \caption{\label{tab: stddev}\textbf{Ergebnisse der Scaffold-Berechnung.}
  Die Anzahl der eingehenden Contigs jedes Datensatzes ist angegeben. Die
  SGA-Ergebnisse sind als absolute Werte aufgetragen, die gt
  Scaffolder-Ergebnisse als relative Faktoren zu ersteren.

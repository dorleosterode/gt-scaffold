...
extrahieren. Dafür diente die Methode des Programms DistEst von ABySS als Vorlage. Aus Zeitgründen ist es nicht gelungen ein Modul zu
...
Danach wird von der Verteilung eine Wahrscheinlichkeitsfunktion für die Fragmentlängen abgeleitet. Im nächsten Schritt versucht man, den paarweisen Abstand der Contigs durch eine Maximum-Likelihood-Methode abzuschätzen.
Dabei werden für ein Contig-Paar die zugehörigen Read-Paare ermittelt und deren Fragmentlänge berechnet unter der Annahme, dass die beiden Contigs ohne Lücke direkt aneinander angrenzen. Als nächstes durchläuft man mehrere Iterationen, bei der jeweils die Fragmentlängen um einen Faktor (min\_dist bis max\_dist) korrigiert werden. Des Weiteren erfolgt eine Aufsummierung der logarithmischen Wahrscheinlichkeiten (log odds) der korrigierten Fragmentlängen multipliziert mit dessen Häufigkeiten in jedem Durchlauf. Schließlich wählt man als Abstand zwischen den Contigs den Faktor, bei dem die Wahrscheinlichkeitssumme ein Maximum erreicht.
...
über die Astatistik und die berechneten Kopiezahlen der Contigs
übergeben werden. Diese Option wird für die Kompabilität mit der
...
Scaffolder ohne eine extra Astatistik-Datei aufgerufen, so wird
...
\item Bei der Konstruktion des Graphen werden zwischen zwei Knoten
  ...
  Wenn es mehr als ein mögliches Kantepaar gibt, wird das Kantenpaar mit der
...
Zusätzlich sind die Referenzgenome hinreichend groß um ein
interessantes Problem darzustellen. Kleinere Genome konnten teilweise
von der SGA-Pipeline schon bei der Assemblierung vollständig
rekonstruiert werden.

Mit dem Sequenzierungssimulator ART~\cite{Huang:2012kq} wurde für
jedes der Genome eine \textit{Paired-End}-Sequenzierung mit einem
Illumina Sequenzierer simuliert. Dabei wurde die Read-Länge auf 150
bp, die Fragmentlänge auf 400 bp $\pm$ 10 bp und die Coverage auf 20X
festgelegt.

Die so erhaltenen \textit{Paired-End}-Reads wurden mit der
SGA-Pipeline assembliert und das Ergebnis für das Scaffolding
verwendet.
...
Laufzeit und mehr Speicheplatz benötigt, wie aus Tabelle \ref{tab:
  Vergleich} und Abbildung \ref{abb: Zeit} ersichtlich
wird. \todo{gehört dieser Abschnitt evtl in die Diskussion?}
...
\begin{algorithm}[H]
  ...
  \caption{\label{alg: Pfade}Berechnung der Walks (Schritt 6.1)}
\end{algorithm}

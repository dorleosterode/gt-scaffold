...
\begin{frame} % SK: Abstände überarbeiten
  ...
  \begin{columns}
    \begin{column}{.45\textwidth}
      \tiny SGA Pipeline: SGA (C++, Python, Perl), Abyss (C++), Samtools (C)
    \end{column}
    \begin{column}{.45\textwidth}
     \tiny gt Pipeline: GenomeTools (C)
    \end{column}
  \end{columns}
  ...
  \begin{columns}
    \begin{column}{.45\textwidth}
      \begin{tikzpicture}[node distance=.75cm]
        ...
        \node[prog, right of=prep, xshift=1cm, fill=red!40] (bindex) {index};
